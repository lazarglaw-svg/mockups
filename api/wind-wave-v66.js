...
const PATCH = `
<style id="v66-ios-hero-fix">
/* Definitive mobile hero handoff. No viewport-height units. */
@media (max-width: 820px) and (orientation: portrait) {
  .hero {
    ...
    background-image:
      linear-gradient(
        ...
        rgba(3,10,14,.10) 0%,
        rgba(3,10,14,0) 56%,
        rgba(52,111,119,.06) 72%,
        rgba(52,111,119,.36) 88%,
        rgba(52,111,119,.72) 100%
      ),
  ...
  .hero::after {
    ...
    background: linear-gradient(
      180deg,
      rgba(52,111,119,0) 0%,
      rgba(52,111,119,.05) 16%,
      rgba(52,111,119,.20) 34%,
      rgba(52,111,119,.48) 55%,
      rgba(52,111,119,.78) 76%,
      #346f77 100%
    ) !important;
  }
...
/* Extra Safari guard: older iPhone Safari must not reintroduce svh/vh sizing. */
...
(function () {
  ...
  function applyIOSHeroFix() {
    ...
    if (overlay) {
      overlay.style.setProperty('bottom', '34px', 'important');
  ...
  window.addEventListener('resize', function () { clearTimeout(window.__wwIOSFix); window.__wwIOSFix = setTimeout(applyIOSHeroFix, 100); });
})();
</script>`;
...
export default async function handler(req, res) {
  try {
    const response = await fetch(`${RAW_INDEX}?v=${Date.now()}`, {
      headers: { 'User-Agent': 'wind-wave-preview-v66' },
      cache: 'no-store'
    });
    ...
    res.setHeader('X-Wind-Wave-Version', 'v66-ios');
